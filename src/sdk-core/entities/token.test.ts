...
describe("Token", () => {
  ...
  describe("#constructor", () => {
    ...
    it("fails with 256 decimals", () => {
      expect(() => new Token({ address: ADDRESS_ONE, decimals: 256, standard: "EXT" }).address).toThrow("DECIMALS");
    });
    it("fails with non-integer decimals", () => {
      expect(() => new Token({ address: ADDRESS_ONE, decimals: 1.5, standard: "EXT" }).address).toThrow("DECIMALS");
    });
  });
  ...
  describe("#equals", () => {
    ...
    it("true on reference equality", () => {
      const token = new Token({ address: ADDRESS_ONE, decimals: 18, standard: "EXT" });
